...
void drawSpeechBubble(int tip_x, int tip_y, const char* line1, const char* line2) {
  ...
  const int pad_y = 6;
  const int gap = 4;
  const int tw1 = static_cast<int>(strlen(l1)) * 12;  // size 2
...
void handleRoot() {
  String html = F("<!doctype html><html><body style='font-family:sans-serif;background:#000;color:#fff'>"
                  "<h1>Cursor Buddy</h1><p>POST /event JSON</p><pre>");
  html += "ip: ";
...
void setup() {
  Serial.begin(115200);
  delay(400);
  Serial.println("Cursor buddy + WiFi events");
...
void loop() {
  ...
  int y = 150 + bob + static_cast<int>(constrain(smooth_ay, -1.0f, 1.0f) * 6);
  y = constrain(y, 120, LCD_HEIGHT - spr_h - 28);

  // Globo sobre el cubo mientras haya evento (o siempre el status actual)
  ...
  gfx->setTextSize(1);
  gfx->setTextColor(COL_MUTED);
  gfx->setCursor(12, LCD_HEIGHT - 12);
  gfx->print(wifi_ok ? "cursor-buddy.local" : "check 2.4ghz");
